...
from glances.plugins.sensors.sensor.glances_nvme import PluginModel as NvmePluginModel
...
class PluginModel(GlancesPluginModel):
    ...
    def __init__(self, args=None, config=None):
        ...
        start_duration.reset()
        nvme_plugin = NvmePluginModel(args=args, config=config)
        logger.debug(f"NVMe sensor plugin init duration: {start_duration.get()} seconds")
        ...
        self.sensors_grab_map['nvme'] = nvme_plugin
...
class GlancesGrabSensors:
    ...
    def __fetch_psutil(self) -> Dict[str, list]:
        ...
        if sensor_type == 'nvme':
            data = self.sensors_grab_map['nvme'].update()
            data = self.__set_type(data, 'nvme')
